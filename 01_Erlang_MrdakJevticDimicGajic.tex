...
\label{sec:nastanak}
Godine 1981.  je oformljena nova laboratorija, Erikson CSLab (eng.~{\em The Ericsson CSLab}) u okviru firme Erikson sa
...
svojih softverskih rešenja. Neke od njih su: Erikson, Motorola, Votsap (eng.~{\em }), 
...
dijalekt Prologa, a kasnije je zbog svoje kompleksnosti i sveobuhvatnosti evaluirao
...
osim ako se u interpreteru ne pozove funkcija f() koja sve promenjive načini slobodnim \cite{book_fred}. 
...
Pri unifikaciji na raspolaganju je i posebna anonimna promenljiva \_ koju koristimo kada nas neka vrednost ne zanima i ne želimo nijednu promenjivu da vežemo za tu vrednost. U sledećem primeru prve tri linije pokazuju uspešnu unifikaciju, dok je u poslednjoj pokušana unifikacija X sa 51, što nije uspelo kako je X već vezano za \{137, 42\}.
...
max(X, Y) -> Y.
\end{minted}
Funkcija {\em max} je definisana preko poklapanja obrazaca uz korišćenje čuvara jednog uslova koji određuje povratnu vrednost. U narednom delu formalizovaćemo šta su funkcije u Erlangu.
...
%Slanje
Erlang omogućava jednostavno kreiranje novog procesa pozivom funkcije {\em spawn} koja vraća pid (eng.~{\em process identifier}) na osnovu kojeg svaki proces može da razlikuje ostale procese.
\begin{minted}{erlang}
...
{\em Receive} vraća vrednost poslednjeg izraza iz izvršene akcije.
...
Funkcijom {\mintinline{erlang}{register(name, Pid)}} atom {\em name} se vezuje za {\em Pid} i na dalje ga možemo identifikovati preko dodeljenog atoma.\\
...
i onda se povezuje sa procesom {\em Pid} uz pomoć funkcije {\em link(Pid)}. 
...
za neke operativne sisteme zasnovane na Linuksovom jezgru i pokretanje na jednom od njih, kao i instalacija za Vindouz.
...
Na operativnom sistemu {\em Vindouz}, Erlang se može instalirati preuzimanjem binarne datoteke sa zvaničnog sajta \cite{sajt} programskog jezika. Posle duplog klika na {\em .exe} fajl samo je potrebno ispratiti uputstva. Pokretanje interpretatora se vrši na isti način kao i na Linuks sistemima.
...
a da bi one mogle da se koriste izvan modula potrebno je da ih navedemo u export naredbi. start/0 označava da funkcija \textit{start} prima nula argumenata.
...
{\em map(F, List)} vraća novu listu dobijenu primenom funkcije {\em F} na svaki element liste {\em L}.
\begin{minted}{erlang}
...
U klijentskoj funkciji pošiljalac mora da uključi svoju adresu sa {\em self()} i pošalje zahtev na serverski proces.
Potom čeka odgovor koji je namenjen njemu, odnosno prihvata odgovor kada se poklopi obrazac torke \{Pid, Response\}. Tačnije, {\em Pid} je vezan, a {\em Response} je slobodna promenljiva koja će biti vezana kada stigne odgovor.
\begin{minted}{erlang}
klijent(Pid, Request) ->
    Pid ! {self(), Request},
    receive
